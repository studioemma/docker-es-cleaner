...
INDICES_TEXT=`curl -s "$ELASTICSEARCH/_cat/indices?pretty=true" | grep $GREP | cut -d" " -f3 | sort -r`
...
if [ ${#INDEX[@]} -gt $KEEP ]; then
  for index in ${INDEX[@]:$KEEP};do
    ...
    if [ -n "$index" ]; then
      if [ -z "$LOGFILE" ]; then
        [[ $DRYRUN -eq 0 ]] && curl -s -XDELETE "$ELASTICSEARCH/$index/" > /dev/null
      else
        echo `date "+[%Y-%m-%d %H:%M] "`" Deleting index: $index." >> $LOGFILE
        [[ $DRYRUN -eq 0 ]] && curl -s -XDELETE "$ELASTICSEARCH/$index/" >> $LOGFILE
        echo >> $LOGFILE
      fi
    fi
  done
